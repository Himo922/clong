...
export const About = ({ className, styles }) => {
  ...
  return (
    <div
      id="#About"
      className={clsx(
        "z-10 bg-blue-900 w-screen m-0 min-h-[600px] p-8",
        className
      )}
      style={styles}
      ref={ref}
      ...
      <Section className="" title="About" color="text-orange-700">
        <Container>
          <motion.div
            animate={animation}
            className={clsx(
              "flex flex-row m-0 p-0 my-15 z-10 items-center p-5",
              "md:mx-8 md:px-10 md:px-25 "
            )}
          >
            <div>
              <p className="text-base max-w-[85%] md:text-xl dark:text-white-700 text-black-700">
                Hi, I am CLong Lam. A Software Developer and Media Designer.
                Currently bassed in Vancourver. Love to do experimental stuff,
                play with coding & hardware. 4-Year Work Experience from
                front-end to full-stack. View my Works and projects at Github |
                Code Pen. "
              </p>
